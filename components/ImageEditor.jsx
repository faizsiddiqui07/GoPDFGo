...
const ImageEditor = ({ toolId }) => {
  ...
  // combine-images: lay several photos onto ONE A4 page (front/back of an ID),
  // dragging and resizing each freely. Placement is stored per file as
  // fractions of the page, so it survives orientation changes and re-renders.
  const [combineOrient, setCombineOrient] = useState("portrait"); // portrait | landscape
  const [combineFormat, setCombineFormat] = useState("image/jpeg");
  const [combinePlace, setCombinePlace] = useState({}); // { [uid]: {x,y,w} }
  const [combineMeta, setCombineMeta] = useState({}); // { [uid]: {natW,natH} }
  const [combineSelected, setCombineSelected] = useState(null); // uid
  ...
  // Stable id per File object for the combine A4 editor, so a placement stays
  // attached to its photo across reorders, adds and removes. Keyed by object
  // identity, so it needs no change to the shape of `files`.
  const fileUidRef = useRef(new WeakMap());
  const fileUidCounter = useRef(0);
  // The A4 preview box, for pointer-to-fraction math during drag/resize.
  const combineCanvasRef = useRef(null);
  // A hidden file input that is always mounted while the combine tool has
  // photos, so "Add more photos" never calls .click() on a missing element
  // (the dropzone's own input unmounts once files exist).
  ...
  // Stable id for a File, assigned lazily and remembered by object identity.
  const uidOf = (file) => {
    let id = fileUidRef.current.get(file);
    if (!id) {
      id = `f${++fileUidCounter.current}`;
      fileUidRef.current.set(file, id);
    }
    return id;
  };

  // A4 at ~150 DPI: crisp enough to read an ID, comfortably under the 5000px
  // canvas cap a phone can allocate. The on-screen page uses the same ratio.
  const A4_W = 1240;
  const A4_H = 1754;
  const pageWpx = combineOrient === "portrait" ? A4_W : A4_H;
  const pageHpx = combineOrient === "portrait" ? A4_H : A4_W;
  const pageAR = pageWpx / pageHpx; // width / height

  // Thumbnails, one per file, used both by the left-panel list and as the
  // draggable images on the page. Rebuilt when the file set changes, with the
  // previous batch revoked so a long session cannot leak blob URLs.
  ...
  // Measure every new photo and give it a starting spot on the page. Existing
  // placements are preserved, so adding a third photo never disturbs the two
  // already arranged. New photos cascade slightly so they don't land exactly
  // on top of each other.
  useEffect(() => {
    if (!isCombineTool || files.length === 0) return;
    let cancelled = false;
    (async () => {
      const measured = {};
      for (const f of files) {
        const id = uidOf(f);
        if (combineMeta[id]) continue;
        try {
          const bmp = await createImageBitmap(f, {
            imageOrientation: "from-image",
          });
          measured[id] = { natW: bmp.width, natH: bmp.height };
          bmp.close && bmp.close();
        } catch {
          /* unreadable file — skip, download will skip it too */
        }
      }
      if (cancelled) return;
      if (Object.keys(measured).length)
        setCombineMeta((prev) => ({ ...prev, ...measured }));
      setCombinePlace((prev) => {
        const next = { ...prev };
        let added = 0;
        files.forEach((f) => {
          const id = uidOf(f);
          if (next[id]) return;
          const shift = (Object.keys(next).length % 5) * 0.05;
          next[id] = { x: 0.08 + shift, y: 0.08 + shift, w: 0.5 };
          added++;
        });
  ...
  // Drag to move, or drag the corner to resize — one photo at a time, in
  // fractions of the page so the result is resolution-independent. Mirrors the
  // Sign-PDF placement handles.
  const combineDrag = (e, uid, mode) => {
    e.preventDefault();
    e.stopPropagation();
    const box = combineCanvasRef.current;
    if (!box || !combinePlace[uid]) return;
    setCombineSelected(uid);
    const rect = box.getBoundingClientRect();
    const m = combineMeta[uid];
    const imgAR = m ? m.natW / m.natH : 1; // width / height of the photo
    const orig = { ...combinePlace[uid] };
    const startX = e.clientX;
    const startY = e.clientY;
    const pid = e.pointerId;
    // Guard the call itself, not just its existence: setPointerCapture throws
    // when the pointer isn't active, and an uncaught throw here would abort the
    // drag before the move listeners below are attached.
    try {
    ...
    // Height as a fraction of the page, from a width fraction: the photo keeps
    // its aspect ratio, and the page is taller than wide (or vice-versa).
    const hFrac = (w) => (w * pageAR) / imgAR;
    const move = (ev) => {
      ...
      setCombinePlace((prev) => {
        const next = { ...prev };
        if (mode === "move") {
          const hF = hFrac(orig.w);
          next[uid] = {
            ...orig,
            x: clamp(orig.x + dxF, 0, Math.max(0, 1 - orig.w)),
            y: clamp(orig.y + dyF, 0, Math.max(0, 1 - hF)),
          };
        } else {
          // Cap the width so the scaled height can't spill past the bottom edge.
          const maxByBottom = ((1 - orig.y) * imgAR) / pageAR;
          const w = clamp(
            orig.w + dxF,
            0.08,
            Math.max(0.08, Math.min(1 - orig.x, maxByBottom)),
          );
          next[uid] = { ...orig, w };
        }
        return next;
  ...
  // "Add more photos" must APPEND. The normal file flow replaces the whole set,
  // which is right for every other tool but would wipe the page here.
  const addCombineImages = (fileList) => {
  ...
  // Compose the A4 page at full resolution and hand back a single image.
  const downloadCombinedA4 = async () => {
    if (files.length === 0) return;
    setIsProcessing(true);
    setImgError(null);
    try {
      ...
      const ctx = canvas.getContext("2d");
      ctx.imageSmoothingQuality = "high";
      // It's a sheet of paper: white behind everything, both formats.
      ctx.fillStyle = "#FFFFFF";
      ctx.fillRect(0, 0, pageWpx, pageHpx);
      for (const f of files) {
        const id = uidOf(f);
        const p = combinePlace[id];
        const m = combineMeta[id];
        if (!p || !m) continue;
        const bmp = await createImageBitmap(f, {
          imageOrientation: "from-image",
        });
        const w = p.w * pageWpx;
        const h = w * (m.natH / m.natW);
        ctx.drawImage(bmp, p.x * pageWpx, p.y * pageHpx, w, h);
        bmp.close && bmp.close();
      }
      const blob = await new Promise((res) =>
        canvas.toBlob(res, combineFormat, 0.92),
      );
      ...
      console.error(e);
      setImgError(
        "Could not build the page. One of the photos may be corrupted or in a format this browser cannot read.",
      );
    } finally {
      setIsProcessing(false);
    }
            ...
            <div className="space-y-6 h-full flex flex-col">
              ...
              {isBatchMode ? (
                ...
                <>
                  ...
                  {/* Always mounted while photos exist, so Add more never
                      clicks a missing element. */}
                  <input
                    ref={addMoreInputRef}
                    type="file"
                    multiple
                    ...
                    onChange={(e) => addCombineImages(e.target.files)}
                  />
                  <button
                  ...
                  {files.length < 2 && (
                    <div className="flex items-start gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-100 rounded-lg p-3">
                      ...
                      <span>
                        Add another photo — the front and back of your ID, for
                        example — then drag each one where you want it on the
                        page.
                      </span>
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { v: "portrait", label: "Portrait", hint: "Tall A4" },
                        { v: "landscape", label: "Landscape", hint: "Wide A4" },
                      ].map((o) => (
                        <button
                          key={o.v}
                          onClick={() => setCombineOrient(o.v)}
                  ...
                  >
                    ...
                    {isProcessing ? "Building page..." : "Download A4 Page"}
                  </button>
                </>
              ) : (
                <>
          ...
          {isBatchMode ? (
            ...
            files.length > 0 ? (
              <div className="w-full h-full flex flex-col items-center justify-center animate-fade-in gap-3">
                {/* The A4 page. Tapping empty space deselects. */}
                <div
                  ref={combineCanvasRef}
                  onPointerDown={() => setCombineSelected(null)}
                  className="relative bg-white rounded-lg shadow-md border border-slate-200 overflow-hidden select-none touch-none mx-auto"
                  style={{
                    aspectRatio: `${pageWpx} / ${pageHpx}`,
                    maxHeight: "70vh",
                    maxWidth: "100%",
                    height: combineOrient === "portrait" ? "70vh" : "auto",
                    width: combineOrient === "landscape" ? "100%" : "auto",
                  }}
                >
                  {files.map((f, i) => {
                    const id = uidOf(f);
                    const p = combinePlace[id];
                    if (!p) return null;
                    const selected = combineSelected === id;
                    return (
                      <div
                        key={id}
                        onPointerDown={(e) => combineDrag(e, id, "move")}
                        className={`absolute cursor-move ${
                          selected
                            ? "ring-2 ring-[#FF9933] z-30"
                      ...
                      >
                        ...
                        {/* Resize handle — its own 44px touch target. */}
                        <div
                          onPointerDown={(e) => combineDrag(e, id, "resize")}
                          className="absolute -bottom-3 -right-3 w-11 h-11 flex items-center justify-center cursor-nwse-resize touch-manipulation"
                          style={{ touchAction: "none" }}
                        >
                          <span className="w-5 h-5 bg-white border-2 border-[#FF9933] rounded-full shadow" />
                        </div>
                      </div>
                    );
                  })}
                ...
                <p className="text-xs text-slate-500 text-center px-4">
                  Drag a photo to move it · drag the orange corner to resize.
                  {fileSize ? ` Last saved: ${fileSize}.` : ""}
                </p>
              </div>
            ) : (
              <div className="text-slate-400 flex flex-col items-center">
                <ImageIcon size={64} className="mb-4 opacity-30" />
                <p className="font-medium">Add your photos</p>
                <p className="text-xs mt-1">
                  Place them on an A4 page, then download
                </p>
              </div>
            )
          ) : previewUrl ? (
